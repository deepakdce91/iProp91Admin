...
import ArticlesForm from "../../../components/frontendPreview/ArticlesForm";
...
function Index() {
  ...
  const setModeToDisplay = () => {
    setMode("display");
    fetchAllArticles(userId, userToken);
  };
  ...
  const fetchArticle = async (id) => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/articles/fetchArticle/${id}?userId=${userId}`,
        {
          headers: {
  ...
  const fetchAllArticles = (userId, userToken) => {
    axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/articles/fetchAllArticles?userId=${userId}`,
        {
          headers: {
            "auth-token": userToken,
          },
  ...
  const deleteArticlessById = async (id) => {
    // Make the DELETE request
    await axios
      .delete(
        `${process.env.REACT_APP_BACKEND_URL}/api/articles/deleteArticle/${id}?userId=${userId}`,
        {
          headers: {
            "auth-token": userToken,
          },
  ...
  useEffect(() => {
    try {
      ...
      if (token) {
        ...
        setUserToken(token);
        fetchAllArticles(decoded.userId, token);
      }
    } catch (error) {
      console.log(error);
    }
