...
function Index() {
  ...
  const setModeToDisplay = () => {
    setMode("display");
    fetchAllComparisons(userId, userToken);
  };
  ...
  const fetchComparison = async (id) => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/comparisons/fetchComparison/${id}?userId=${userId}`,
        {
          headers: {
  ...
  const fetchAllComparisons = (userId, userToken) => {
    axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/comparisons/fetchAllComparisons?userId=${userId}`,
        {
          headers: {
            "auth-token": userToken,
          },
  ...
  const deleteComparisonById = async (id) => {
    ...
    await axios
      .delete(
        `${process.env.REACT_APP_BACKEND_URL}/api/comparisons/deleteComparison/${id}?userId=${userId}`,
        {
          headers: {
            "auth-token": userToken,
          },
  ...
  useEffect(() => {
    try {
      ...
      if (token) {
        ...
        setUserToken(token);
        fetchAllComparisons(decoded.userId, token);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <Box m="20px">
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header
          title="Comparisons Section"
          subtitle={
            mode === "add"
              ? "Add Comparison"
              : mode === "edit"
              ? "Edit Comparison"
              : "Manage Comparisons here"
          }
        />
