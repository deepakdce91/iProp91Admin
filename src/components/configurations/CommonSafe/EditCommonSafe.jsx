...
// ------------------
const EditCommonSafe = ({ userId, userToken, commonSafeId, fieldName }) => {
  ...
  const fetchFieldData = async () => {
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/commonSafe/getDocuments/${commonSafeId}/${fieldName}?userId=${userId}`,
        {
          headers: {
  ...
  const addFile = async (body) => {
    await axios
      .post(
        `${process.env.REACT_APP_BACKEND_URL}/api/commonSafe/addDocument/${commonSafeId}/${fieldName}?userId=${userId}`,
        body,
        {
          headers: {
            "auth-token": userToken,
  ...
  const deleteFile = async (id) => {
    await axios
      .delete(
        `${process.env.REACT_APP_BACKEND_URL}/api/commonSafe/removeDocument/${commonSafeId}/${fieldName}/${id}?userId=${userId}`,
        {
          headers: {
            "auth-token": userToken,
          },
  ...
  const uploadFileToCloud = async (commonSafeId, fieldName, myFile) => {
    const myFileName = removeSpaces(myFile.name); // removing blank space from name
    const myPath = `commonSafe/${commonSafeId}/${fieldName}/${myFileName}`;
    try {
      const uploadParams = {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      let cloudFileDetails = await uploadFileToCloud(commonSafeId, fieldName, item);
