...
function TermsAndConditionsForm({ editData, setModeToDisplay, userToken, userId }) {
  ...
  const handleSubmit = () => {
    if (addData.text !== "") {
      if (editData) {
        axios
          .put(
            `${process.env.REACT_APP_BACKEND_URL}/api/termsAndConditions/update/${editData._id}?userId=${userId}`,
            addData,
            {
              headers: {
        ...
        axios
          .post(
            `${process.env.REACT_APP_BACKEND_URL}/api/termsAndConditions/add?userId=${userId}`,
            addData,
            {
              headers: {
                "auth-token": userToken,
      ...
      toast.error("Privacy Policy text is required.");
    }
  };
